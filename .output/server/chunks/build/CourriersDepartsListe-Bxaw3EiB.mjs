...
import { ref, mergeProps, withCtx, createTextVNode, createVNode, unref, createBlock, openBlock, Fragment, renderList, toDisplayString, withDirectives, vModelText, vModelSelect, useSSRContext } from 'vue';
...
const _sfc_main = {
  ...
  setup(__props) {
    ...
    const transformCourriers = (response) => {
      ...
      return response.data.map((courrier) => {
        ...
        const initiateursList = (courrier.initiateurs || []).map((i) => {
          var _a2, _b2, _c2, _d2;
          const nom = ((_a2 = i.user) == null ? void 0 : _a2.nom) || "";
          const prenom = ((_b2 = i.user) == null ? void 0 : _b2.prenom) || "";
          const code = ((_c2 = i.entite) == null ? void 0 : _c2.code) || ((_d2 = i.entite) == null ? void 0 : _d2.libelle) || "";
        ...
        return {
          ...
          type_document: ((_h = (_g = courrier.document) == null ? void 0 : _g.type_document) == null ? void 0 : _h.libelle) || "",
          large_diffusion: ((_i = courrier.document) == null ? void 0 : _i.large_diffusion) || false,
    ...
    const { data: courriers, loading, error } = useApi("/courriers-departs", {
    ...
    return (_ctx, _push, _parent, _attrs) => {
      ...
      _push(`<div${ssrRenderAttrs(mergeProps({ class: "min-h-screen bg-slate-100 p-6 font-sans" }, _attrs))} data-v-c326b5a2><div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-6" data-v-c326b5a2><div data-v-c326b5a2><h1 class="text-2xl font-bold text-slate-900 tracking-tight" data-v-c326b5a2>Courriers D\xE9parts</h1><p class="text-sm text-slate-500" data-v-c326b5a2>Gestion et suivi des courriers sortants</p></div>`);
      ...
      _push(`</div>`);
      if (unref(loading)) {
        _push(`<div class="flex flex-col items-center justify-center py-20 gap-4 text-slate-500" data-v-c326b5a2><div class="w-8 h-8 border-4 border-slate-200 border-t-indigo-600 rounded-full animate-spin" data-v-c326b5a2></div><span class="text-sm font-medium" data-v-c326b5a2>Chargement des donn\xE9es...</span></div>`);
      } else if (unref(error)) {
        _push(`<div class="flex items-center gap-4 p-5 bg-red-50 border border-red-200 rounded-xl max-w-2xl" data-v-c326b5a2><svg class="w-8 h-8 text-red-600 shrink-0" viewBox="0 0 20 20" fill="currentColor" data-v-c326b5a2><path fill-rule="evenodd" d="M18 10a8 8 0 11-16 0 8 8 0 0116 0zm-7 4a1 1 0 11-2 0 1 1 0 012 0zm-1-9a1 1 0 00-1 1v4a1 1 0 102 0V6a1 1 0 00-1-1z" clip-rule="evenodd" data-v-c326b5a2></path></svg><div class="flex-1" data-v-c326b5a2><p class="text-sm font-bold text-red-900" data-v-c326b5a2>Erreur de chargement</p><p class="text-xs text-red-700" data-v-c326b5a2>${ssrInterpolate(unref(error))}</p></div><button class="px-4 py-2 text-xs font-bold text-red-600 bg-white border border-red-200 rounded-lg hover:bg-red-50 transition-colors shrink-0" data-v-c326b5a2> R\xE9essayer </button></div>`);
      } else {
        _push(ssrRenderComponent(_sfc_main$1, {
          ...
          "advanced-filters": withCtx(({ filters, onFilter }, _push2, _parent2, _scopeId) => {
            if (_push2) {
              _push2(`<div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4" data-v-c326b5a2${_scopeId}><div data-v-c326b5a2${_scopeId}><label class="block text-[11px] font-bold text-slate-500 uppercase tracking-wider mb-1.5" data-v-c326b5a2${_scopeId}>Recherche</label><input${ssrRenderAttr("value", filters.search)} placeholder="R\xE9f\xE9rence, N\xB0, Objet..." class="w-full px-3 py-2 text-xs text-slate-900 bg-white border border-slate-300 rounded-lg focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500 outline-none transition-all" data-v-c326b5a2${_scopeId}></div><div data-v-c326b5a2${_scopeId}><label class="block text-[11px] font-bold text-slate-500 uppercase tracking-wider mb-1.5" data-v-c326b5a2${_scopeId}>Type de document</label><select class="w-full px-3 py-2 text-xs text-slate-900 bg-white border border-slate-300 rounded-lg focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500 outline-none transition-all" data-v-c326b5a2${_scopeId}><option value="" data-v-c326b5a2${ssrIncludeBooleanAttr(Array.isArray(filters.type_document_id) ? ssrLooseContain(filters.type_document_id, "") : ssrLooseEqual(filters.type_document_id, "")) ? " selected" : ""}${_scopeId}>Tous les types</option><!--[-->`);
              ssrRenderList(documentTypesList.value, (type) => {
                _push2(`<option${ssrRenderAttr("value", type.id)} data-v-c326b5a2${ssrIncludeBooleanAttr(Array.isArray(filters.type_document_id) ? ssrLooseContain(filters.type_document_id, type.id) : ssrLooseEqual(filters.type_document_id, type.id)) ? " selected" : ""}${_scopeId}>${ssrInterpolate(type.libelle)}</option>`);
              });
              _push2(`<!--]--></select></div><div data-v-c326b5a2${_scopeId}><label class="block text-[11px] font-bold text-slate-500 uppercase tracking-wider mb-1.5" data-v-c326b5a2${_scopeId}>Type de d\xE9part</label><select class="w-full px-3 py-2 text-xs text-slate-900 bg-white border border-slate-300 rounded-lg focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500 outline-none transition-all" data-v-c326b5a2${_scopeId}><option value="" data-v-c326b5a2${ssrIncludeBooleanAttr(Array.isArray(filters.type_depart) ? ssrLooseContain(filters.type_depart, "") : ssrLooseEqual(filters.type_depart, "")) ? " selected" : ""}${_scopeId}>Tous les types</option><option value="interne" data-v-c326b5a2${ssrIncludeBooleanAttr(Array.isArray(filters.type_depart) ? ssrLooseContain(filters.type_depart, "interne") : ssrLooseEqual(filters.type_depart, "interne")) ? " selected" : ""}${_scopeId}>Interne</option><option value="externe" data-v-c326b5a2${ssrIncludeBooleanAttr(Array.isArray(filters.type_depart) ? ssrLooseContain(filters.type_depart, "externe") : ssrLooseEqual(filters.type_depart, "externe")) ? " selected" : ""}${_scopeId}>Externe</option></select></div><div data-v-c326b5a2${_scopeId}><label class="block text-[11px] font-bold text-slate-500 uppercase tracking-wider mb-1.5" data-v-c326b5a2${_scopeId}>Date de d\xE9part (d\xE9but)</label><input${ssrRenderAttr("value", filters.date_depart_from)} type="date" class="w-full px-3 py-2 text-xs text-slate-900 bg-white border border-slate-300 rounded-lg focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500 outline-none transition-all" data-v-c326b5a2${_scopeId}></div></div><div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mt-4" data-v-c326b5a2${_scopeId}><div data-v-c326b5a2${_scopeId}><label class="block text-[11px] font-bold text-slate-500 uppercase tracking-wider mb-1.5" data-v-c326b5a2${_scopeId}>Date de d\xE9part (fin)</label><input${ssrRenderAttr("value", filters.date_depart_to)} type="date" class="w-full px-3 py-2 text-xs text-slate-900 bg-white border border-slate-300 rounded-lg focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500 outline-none transition-all" data-v-c326b5a2${_scopeId}></div><div data-v-c326b5a2${_scopeId}><label class="block text-[11px] font-bold text-slate-500 uppercase tracking-wider mb-1.5" data-v-c326b5a2${_scopeId}>Large diffusion</label><select class="w-full px-3 py-2 text-xs text-slate-900 bg-white border border-slate-300 rounded-lg focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500 outline-none transition-all" data-v-c326b5a2${_scopeId}><option value="" data-v-c326b5a2${ssrIncludeBooleanAttr(Array.isArray(filters.large_diffusion) ? ssrLooseContain(filters.large_diffusion, "") : ssrLooseEqual(filters.large_diffusion, "")) ? " selected" : ""}${_scopeId}>Tous</option><option value="true" data-v-c326b5a2${ssrIncludeBooleanAttr(Array.isArray(filters.large_diffusion) ? ssrLooseContain(filters.large_diffusion, "true") : ssrLooseEqual(filters.large_diffusion, "true")) ? " selected" : ""}${_scopeId}>Oui</option><option value="false" data-v-c326b5a2${ssrIncludeBooleanAttr(Array.isArray(filters.large_diffusion) ? ssrLooseContain(filters.large_diffusion, "false") : ssrLooseEqual(filters.large_diffusion, "false")) ? " selected" : ""}${_scopeId}>Non</option></select></div><div data-v-c326b5a2${_scopeId}><label class="block text-[11px] font-bold text-slate-500 uppercase tracking-wider mb-1.5" data-v-c326b5a2${_scopeId}>Confidentiel</label><select class="w-full px-3 py-2 text-xs text-slate-900 bg-white border border-slate-300 rounded-lg focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500 outline-none transition-all" data-v-c326b5a2${_scopeId}><option value="" data-v-c326b5a2${ssrIncludeBooleanAttr(Array.isArray(filters.confidentiel) ? ssrLooseContain(filters.confidentiel, "") : ssrLooseEqual(filters.confidentiel, "")) ? " selected" : ""}${_scopeId}>Tous</option><option value="true" data-v-c326b5a2${ssrIncludeBooleanAttr(Array.isArray(filters.confidentiel) ? ssrLooseContain(filters.confidentiel, "true") : ssrLooseEqual(filters.confidentiel, "true")) ? " selected" : ""}${_scopeId}>Oui</option><option value="false" data-v-c326b5a2${ssrIncludeBooleanAttr(Array.isArray(filters.confidentiel) ? ssrLooseContain(filters.confidentiel, "false") : ssrLooseEqual(filters.confidentiel, "false")) ? " selected" : ""}${_scopeId}>Non</option></select></div><div data-v-c326b5a2${_scopeId}><label class="block text-[11px] font-bold text-slate-500 uppercase tracking-wider mb-1.5" data-v-c326b5a2${_scopeId}>Date enregistrement (d\xE9but)</label><input${ssrRenderAttr("value", filters.date_from)} type="date" class="w-full px-3 py-2 text-xs text-slate-900 bg-white border border-slate-300 rounded-lg focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500 outline-none transition-all" data-v-c326b5a2${_scopeId}></div></div>`);
            } else {
              return [
                createVNode("div", { class: "grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4" }, [
                  ...
                  createVNode("div", null, [
                    createVNode("label", { class: "block text-[11px] font-bold text-slate-500 uppercase tracking-wider mb-1.5" }, "Type de document"),
                    withDirectives(createVNode("select", {
                      "onUpdate:modelValue": ($event) => filters.type_document_id = $event,
                      class: "w-full px-3 py-2 text-xs text-slate-900 bg-white border border-slate-300 rounded-lg focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500 outline-none transition-all",
                      onChange: onFilter
                    }, [
                      createVNode("option", { value: "" }, "Tous les types"),
                      (openBlock(true), createBlock(Fragment, null, renderList(documentTypesList.value, (type) => {
                        return openBlock(), createBlock("option", {
                          key: type.id,
                          value: type.id
                        }, toDisplayString(type.libelle), 9, ["value"]);
                      }), 128))
                    ], 40, ["onUpdate:modelValue", "onChange"]), [
                      [vModelSelect, filters.type_document_id]
                    ])
                  ]),
                  createVNode("div", null, [
                ...
                createVNode("div", { class: "grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mt-4" }, [
                  createVNode("div", null, [
                  ...
                  createVNode("div", null, [
                    createVNode("label", { class: "block text-[11px] font-bold text-slate-500 uppercase tracking-wider mb-1.5" }, "Large diffusion"),
                    withDirectives(createVNode("select", {
                      "onUpdate:modelValue": ($event) => filters.large_diffusion = $event,
                      class: "w-full px-3 py-2 text-xs text-slate-900 bg-white border border-slate-300 rounded-lg focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500 outline-none transition-all",
                      onChange: onFilter
                    }, [
                      createVNode("option", { value: "" }, "Tous"),
                      createVNode("option", { value: "true" }, "Oui"),
                      createVNode("option", { value: "false" }, "Non")
                    ], 40, ["onUpdate:modelValue", "onChange"]), [
                      [vModelSelect, filters.large_diffusion]
                    ])
                  ]),
                  createVNode("div", null, [
                    createVNode("label", { class: "block text-[11px] font-bold text-slate-500 uppercase tracking-wider mb-1.5" }, "Confidentiel"),
                    withDirectives(createVNode("select", {
                      "onUpdate:modelValue": ($event) => filters.confidentiel = $event,
                      class: "w-full px-3 py-2 text-xs text-slate-900 bg-white border border-slate-300 rounded-lg focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500 outline-none transition-all",
                      onChange: onFilter
                    }, [
                      createVNode("option", { value: "" }, "Tous"),
                      ...
                      [vModelSelect, filters.confidentiel]
                    ])
                  ]),
                  createVNode("div", null, [
                    createVNode("label", { class: "block text-[11px] font-bold text-slate-500 uppercase tracking-wider mb-1.5" }, "Date enregistrement (d\xE9but)"),
                    withDirectives(createVNode("input", {
          ...
          actions: withCtx(({ item }, _push2, _parent2, _scopeId) => {
            if (_push2) {
              _push2(`<div class="flex gap-1.5 justify-end" data-v-c326b5a2${_scopeId}><button title="Voir les d\xE9tails" class="inline-flex items-center justify-center w-8 h-8 bg-amber-50 text-amber-700 border-amber-100 rounded-md hover:bg-amber-200 hover:border-amber-900 transition-all group" data-v-c326b5a2${_scopeId}>`);
              _push2(ssrRenderComponent(_component_Icon, {
          ...
          "cell-source": withCtx(({ value }, _push2, _parent2, _scopeId) => {
            if (_push2) {
              _push2(`<span class="inline-flex px-2.5 py-1 text-[11px] font-bold rounded-full bg-indigo-50 text-indigo-700 border border-indigo-100" data-v-c326b5a2${_scopeId}>${ssrInterpolate(value)}</span>`);
            } else {
              return [
          ...
          "cell-type_document": withCtx(({ value }, _push2, _parent2, _scopeId) => {
            if (_push2) {
              _push2(`<span class="inline-flex px-2.5 py-1 text-[11px] font-bold rounded-full bg-green-50 text-green-700 border border-green-100" data-v-c326b5a2${_scopeId}>${ssrInterpolate(value || "N/A")}</span>`);
            } else {
              return [
          ...
          "cell-large_diffusion": withCtx(({ value }, _push2, _parent2, _scopeId) => {
            if (_push2) {
              if (value) {
                _push2(`<span class="inline-flex px-2.5 py-1 text-[11px] font-bold rounded-full bg-blue-50 text-blue-700 border border-blue-100" data-v-c326b5a2${_scopeId}>Oui</span>`);
              } else {
                _push2(`<span class="inline-flex px-2.5 py-1 text-[11px] font-bold rounded-full bg-gray-50 text-gray-700 border border-gray-100" data-v-c326b5a2${_scopeId}>Non</span>`);
              }
            } else {
              return [
          ...
          "cell-confidentiel": withCtx(({ value }, _push2, _parent2, _scopeId) => {
            if (_push2) {
              if (value) {
                _push2(`<span class="inline-flex px-2.5 py-1 text-[11px] font-bold rounded-full bg-red-50 text-red-700 border border-red-100" data-v-c326b5a2${_scopeId}>Oui</span>`);
              } else {
                _push2(`<span class="inline-flex px-2.5 py-1 text-[11px] font-bold rounded-full bg-gray-50 text-gray-700 border border-gray-100" data-v-c326b5a2${_scopeId}>Non</span>`);
              }
            } else {
          ...
          "cell-reference": withCtx(({ value, item }, _push2, _parent2, _scopeId) => {
            if (_push2) {
              if (item.url) {
                _push2(`<button class="inline-flex items-center gap-1.5 px-2.5 py-0.5 text-xs font-medium text-blue-700 bg-blue-50 hover:bg-blue-100 border border-blue-200 hover:border-blue-300 rounded-md transition-all group"${ssrRenderAttr("title", `Ouvrir le document ${value}`)} data-v-c326b5a2${_scopeId}>`);
                _push2(ssrRenderComponent(_component_Icon, {
                  name: "i-heroicons-document-text",
                  class: "w-3.5 h-3.5 group-hover:scale-110 transition-transform"
                }, null, _parent2, _scopeId));
                _push2(`<span data-v-c326b5a2${_scopeId}>${ssrInterpolate(value)}</span>`);
                _push2(ssrRenderComponent(_component_Icon, {
                ...
                _push2(`<span class="inline-flex items-center px-2.5 py-0.5 text-xs font-medium text-gray-600 bg-gray-100 rounded-md"${ssrRenderAttr("title", value)} data-v-c326b5a2${_scopeId}>`);
                _push2(ssrRenderComponent(_component_Icon, {
                  name: "i-heroicons-document-text",
          ...
          "cell-initiateurs": withCtx(({ value }, _push2, _parent2, _scopeId) => {
            if (_push2) {
              if (value && value.length > 0) {
                _push2(`<div class="flex flex-col gap-1" data-v-c326b5a2${_scopeId}><!--[-->`);
                ssrRenderList(value, (initiateur, index) => {
                  _push2(`<span class="inline-flex items-center gap-1 px-2 py-0.5 text-[11px] font-medium text-purple-700 bg-purple-50 border border-purple-100 rounded-full whitespace-nowrap" data-v-c326b5a2${_scopeId}>`);
                  _push2(ssrRenderComponent(_component_Icon, {
                ...
                _push2(`<span class="text-xs text-slate-400 italic" data-v-c326b5a2${_scopeId}>\u2014</span>`);
              }
            } else {
              return [
...
const __nuxt_component_6 = /* @__PURE__ */ _export_sfc(_sfc_main, [["__scopeId", "data-v-c326b5a2"]]);

export { __nuxt_component_6 as default };
//# sourceMappingURL=CourriersDepartsListe-Bxaw3EiB.mjs.map

...
import { _ as __nuxt_component_2 } from './DocumentRpreview-B3POm1pQ.mjs';
...
import { _ as _export_sfc, c as useRuntimeConfig, n as navigateTo } from './server.mjs';
...
const _sfc_main = {
  ...
  setup(__props) {
    ...
    const arriveeUrl = computed(() => {
      ...
      console.log("\u{1F517} [arriveeUrl] url brute:", raw, "| apr\xE8s trim:", url);
    ...
    const handleView = async (item) => {
      ...
      console.log("\u{1F441}\uFE0F [handleView] courrier complet re\xE7u:", JSON.stringify(courrier, null, 2));
      console.log("\u{1F4C4} [handleView] document:", courrier.document);
      console.log("\u{1F517} [handleView] url du document:", (_a = courrier.document) == null ? void 0 : _a.url);
      selectedCourrier.value = courrier;
      showArriveeDoc.value = false;
      showReponseDoc.value = false;
      reponseData.value = null;
      detailsOpen.value = true;
      const reponses = ((_b = courrier.document) == null ? void 0 : _b.reponses) || [];
      console.log("\u{1F4E8} [handleView] reponses trouv\xE9es:", reponses);
      if (reponses.length) {
        const courierDepartId = (_c = reponses[0]) == null ? void 0 : _c.reponse_id;
        console.log("\u{1F194} [handleView] reponses[0] complet:", reponses[0]);
        console.log("\u{1F194} [handleView] courierDepartId utilis\xE9:", courierDepartId);
        if (courierDepartId) {
    ...
    const loadReponseData = async (documentId) => {
      ...
      try {
        ...
        console.log("\u{1F4E6} [loadReponseData] Tous les courriers-d\xE9parts:", JSON.stringify(allDeparts, null, 2));
        const allData = (allDeparts == null ? void 0 : allDeparts.data) || allDeparts || [];
        const list = Array.isArray(allData) ? allData : [];
        const doc = list.find((cd) => cd.document_id === documentId) || null;
        console.log("\u{1F4C4} [loadReponseData] courrier-d\xE9part trouv\xE9 pour document_id=" + documentId + ":", doc);
        if (!doc) {
          console.error("\u274C Aucun courrier-d\xE9part trouv\xE9 avec document_id:", documentId);
          reponseData.value = null;
          return;
        }
        const depart = (doc == null ? void 0 : doc.courrierDepart) || (doc == null ? void 0 : doc.courrier_depart) || (doc == null ? void 0 : doc.depart) || {};
        const rawUrl = (((_a = doc == null ? void 0 : doc.document) == null ? void 0 : _a.url) || "").trim();
        console.log("\u{1F517} [loadReponseData] url brute:", rawUrl);
        const buildUrl = (raw) => {
          var _a2;
        ...
        reponseData.value = {
          reference: ((_b = doc == null ? void 0 : doc.document) == null ? void 0 : _b.reference) || "Sans r\xE9f\xE9rence",
          objet: ((_c = doc == null ? void 0 : doc.document) == null ? void 0 : _c.objet) || "Non sp\xE9cifi\xE9",
        ...
        console.error("\u274C [loadReponseData] Erreur chargement r\xE9ponse:", e);
        reponseData.value = null;
      } finally {
    ...
    return (_ctx, _push, _parent, _attrs) => {
      ...
      _push(`<div${ssrRenderAttrs(mergeProps({ class: "min-h-screen bg-slate-100 p-6 font-sans" }, _attrs))} data-v-e97cebaa>`);
        ...
        default: withCtx((_, _push2, _parent2, _scopeId) => {
          ...
          if (_push2) {
            if (selectedCourrier.value) {
              _push2(`<div class="flex flex-col max-h-[90vh] bg-white rounded-xl overflow-hidden" data-v-e97cebaa${_scopeId}><div class="flex items-center justify-between px-6 py-4 border-b border-slate-200 shrink-0" data-v-e97cebaa${_scopeId}><div class="flex items-center gap-3" data-v-e97cebaa${_scopeId}><div class="w-8 h-8 rounded-full bg-indigo-100 flex items-center justify-center" data-v-e97cebaa${_scopeId}>`);
              _push2(ssrRenderComponent(_component_Icon, {
                name: "i-heroicons-envelope-open",
                class: "w-4 h-4 text-indigo-600"
              }, null, _parent2, _scopeId));
              _push2(`</div><div data-v-e97cebaa${_scopeId}><h2 class="text-base font-bold text-slate-900" data-v-e97cebaa${_scopeId}>D\xE9tails du courrier arriv\xE9</h2><p class="text-xs text-slate-500" data-v-e97cebaa${_scopeId}>N\xB0 ${ssrInterpolate((_a = selectedCourrier.value.document) == null ? void 0 : _a.numero_enreg)}</p></div></div><button class="w-7 h-7 flex items-center justify-center rounded-full hover:bg-slate-100 transition-colors text-slate-400 hover:text-slate-700" data-v-e97cebaa${_scopeId}>`);
              _push2(ssrRenderComponent(_component_Icon, {
                name: "i-heroicons-x-mark",
                class: "w-4 h-4"
              }, null, _parent2, _scopeId));
              _push2(`</button></div><div class="overflow-y-auto flex-1 p-5 space-y-5" data-v-e97cebaa${_scopeId}><section class="bg-white rounded-xl border border-slate-200 overflow-hidden" data-v-e97cebaa${_scopeId}><div class="flex items-center justify-between px-4 py-2.5 bg-slate-50 border-b border-slate-100" data-v-e97cebaa${_scopeId}><div class="flex items-center gap-2" data-v-e97cebaa${_scopeId}><div class="w-5 h-5 rounded bg-indigo-100 flex items-center justify-center" data-v-e97cebaa${_scopeId}>`);
              _push2(ssrRenderComponent(_component_Icon, {
                name: "i-heroicons-inbox-arrow-down",
                class: "w-3 h-3 text-indigo-600"
              }, null, _parent2, _scopeId));
              _push2(`</div><span class="text-[11px] font-bold text-slate-600 uppercase tracking-widest" data-v-e97cebaa${_scopeId}>Courrier arriv\xE9</span></div><div class="flex items-center gap-1.5" data-v-e97cebaa${_scopeId}><span class="${ssrRenderClass([{
              ...
              }, "inline-flex px-2 py-0.5 text-[10px] font-bold rounded-full border uppercase"])}" data-v-e97cebaa${_scopeId}>${ssrInterpolate(selectedCourrier.value.priority || "Standard")}</span>`);
              if ((_f = (_e = selectedCourrier.value.document) == null ? void 0 : _e.reponses) == null ? void 0 : _f.length) {
                _push2(`<span class="inline-flex items-center gap-1 px-2 py-0.5 text-[10px] font-bold rounded-full bg-green-50 text-green-700 border border-green-200" data-v-e97cebaa${_scopeId}>`);
                _push2(ssrRenderComponent(_component_Icon, {
                ...
                _push2(`<span class="inline-flex items-center gap-1 px-2 py-0.5 text-[10px] font-bold rounded-full bg-amber-50 text-amber-700 border border-amber-200" data-v-e97cebaa${_scopeId}>`);
                _push2(ssrRenderComponent(_component_Icon, {
              ...
              _push2(`</div></div><div class="p-4 space-y-3" data-v-e97cebaa${_scopeId}><div class="grid grid-cols-1 gap-2" data-v-e97cebaa${_scopeId}><div class="flex items-start gap-3 p-3 bg-indigo-50 rounded-lg border border-indigo-100" data-v-e97cebaa${_scopeId}><div class="w-1 h-full min-h-[2rem] rounded-full bg-indigo-400 shrink-0 self-stretch" data-v-e97cebaa${_scopeId}></div><div data-v-e97cebaa${_scopeId}><p class="text-[10px] font-bold text-indigo-400 uppercase tracking-wider mb-0.5" data-v-e97cebaa${_scopeId}>R\xE9f\xE9rence</p><p class="text-sm font-bold text-indigo-900" data-v-e97cebaa${_scopeId}>${ssrInterpolate(((_g = selectedCourrier.value.document) == null ? void 0 : _g.reference) || "Sans r\xE9f\xE9rence")}</p></div></div><div class="flex items-start gap-3 p-3 bg-amber-50 rounded-lg border border-amber-100" data-v-e97cebaa${_scopeId}><div class="w-1 h-full min-h-[2rem] rounded-full bg-amber-400 shrink-0 self-stretch" data-v-e97cebaa${_scopeId}></div><div data-v-e97cebaa${_scopeId}><p class="text-[10px] font-bold text-amber-500 uppercase tracking-wider mb-0.5" data-v-e97cebaa${_scopeId}>Objet</p><p class="text-sm text-gray-800 leading-relaxed" data-v-e97cebaa${_scopeId}>${ssrInterpolate(((_h = selectedCourrier.value.document) == null ? void 0 : _h.objet) || "Non sp\xE9cifi\xE9")}</p></div></div></div><div class="grid grid-cols-2 md:grid-cols-3 gap-2" data-v-e97cebaa${_scopeId}><div class="p-2.5 bg-slate-50 rounded-lg border border-slate-100" data-v-e97cebaa${_scopeId}><p class="text-[10px] font-bold text-slate-400 uppercase tracking-wider mb-0.5" data-v-e97cebaa${_scopeId}>Source</p><p class="text-xs font-semibold text-slate-800" data-v-e97cebaa${_scopeId}>${ssrInterpolate(selectedCourrier.value.service_enreg || "N/A")}</p></div><div class="p-2.5 bg-slate-50 rounded-lg border border-slate-100" data-v-e97cebaa${_scopeId}><p class="text-[10px] font-bold text-slate-400 uppercase tracking-wider mb-0.5" data-v-e97cebaa${_scopeId}>Structure / Usager</p><p class="text-xs text-slate-800" data-v-e97cebaa${_scopeId}>${ssrInterpolate(selectedCourrier.value.structure || selectedCourrier.value.autre_structure || "Non sp\xE9cifi\xE9")}</p></div><div class="p-2.5 bg-slate-50 rounded-lg border border-slate-100" data-v-e97cebaa${_scopeId}><p class="text-[10px] font-bold text-slate-400 uppercase tracking-wider mb-0.5" data-v-e97cebaa${_scopeId}>Type d&#39;arriv\xE9e</p><p class="text-xs text-slate-800" data-v-e97cebaa${_scopeId}>${ssrInterpolate(selectedCourrier.value.type_arrivee || "Non sp\xE9cifi\xE9")}</p></div><div class="p-2.5 bg-slate-50 rounded-lg border border-slate-100" data-v-e97cebaa${_scopeId}><p class="text-[10px] font-bold text-slate-400 uppercase tracking-wider mb-0.5" data-v-e97cebaa${_scopeId}>N\xB0 enregistrement</p><p class="text-xs font-semibold text-slate-800" data-v-e97cebaa${_scopeId}>${ssrInterpolate(((_i = selectedCourrier.value.document) == null ? void 0 : _i.numero_enreg) || "\u2014")}</p></div><div class="p-2.5 bg-slate-50 rounded-lg border border-slate-100" data-v-e97cebaa${_scopeId}><p class="text-[10px] font-bold text-slate-400 uppercase tracking-wider mb-0.5" data-v-e97cebaa${_scopeId}>Date d&#39;enregistrement </p><p class="text-xs text-slate-800" data-v-e97cebaa${_scopeId}>${ssrInterpolate(formatDate((_j = selectedCourrier.value.document) == null ? void 0 : _j.date_enreg) || "\u2014")}</p></div>`);
              if ((_k = selectedCourrier.value.document) == null ? void 0 : _k.date_courrier) {
                _push2(`<div class="p-2.5 bg-slate-50 rounded-lg border border-slate-100" data-v-e97cebaa${_scopeId}><p class="text-[10px] font-bold text-slate-400 uppercase tracking-wider mb-0.5" data-v-e97cebaa${_scopeId}>Date du courrier</p><p class="text-xs text-slate-800" data-v-e97cebaa${_scopeId}>${ssrInterpolate(formatDate((_l = selectedCourrier.value.document) == null ? void 0 : _l.date_courrier))}</p></div>`);
              } else {
                _push2(`<!---->`);
              }
              _push2(`</div><div class="pt-1" data-v-e97cebaa${_scopeId}>`);
              if (arriveeUrl.value) {
                _push2(`<div data-v-e97cebaa${_scopeId}>`);
                if (!showArriveeDoc.value) {
                  _push2(`<button class="inline-flex items-center gap-2 px-3.5 py-2 text-xs font-semibold text-indigo-700 bg-indigo-50 hover:bg-indigo-100 border border-indigo-200 rounded-lg transition-all" data-v-e97cebaa${_scopeId}>`);
                  _push2(ssrRenderComponent(_component_Icon, {
                  ...
                  _push2(`<div class="mt-2 rounded-lg overflow-hidden border border-slate-200" data-v-e97cebaa${_scopeId}>`);
                ...
                _push2(`<div class="inline-flex items-center gap-2 px-3.5 py-2 text-xs font-medium text-slate-400 bg-slate-50 border border-slate-200 rounded-lg cursor-not-allowed" data-v-e97cebaa${_scopeId}>`);
                _push2(ssrRenderComponent(_component_Icon, {
              ...
              if ((_n = (_m = selectedCourrier.value.document) == null ? void 0 : _m.reponses) == null ? void 0 : _n.length) {
                _push2(`<section class="bg-white rounded-xl border border-emerald-200 overflow-hidden" data-v-e97cebaa${_scopeId}><div class="flex items-center justify-between px-4 py-2.5 bg-emerald-50 border-b border-emerald-100" data-v-e97cebaa${_scopeId}><div class="flex items-center gap-2" data-v-e97cebaa${_scopeId}><div class="w-5 h-5 rounded bg-emerald-100 flex items-center justify-center" data-v-e97cebaa${_scopeId}>`);
                _push2(ssrRenderComponent(_component_Icon, {
                  name: "i-heroicons-arrow-uturn-right",
                  class: "w-3 h-3 text-emerald-600"
                }, null, _parent2, _scopeId));
                _push2(`</div><span class="text-[11px] font-bold text-emerald-700 uppercase tracking-widest" data-v-e97cebaa${_scopeId}>Courrier de r\xE9ponse</span></div>`);
                if (reponseData.value && !loadingReponse.value) {
                  _push2(`<span class="inline-flex items-center gap-1 px-2 py-0.5 text-[10px] font-bold rounded-full bg-emerald-100 text-emerald-700 border border-emerald-200" data-v-e97cebaa${_scopeId}>`);
                  _push2(ssrRenderComponent(_component_Icon, {
                    name: "i-heroicons-check-circle",
                    class: "w-3 h-3"
                  }, null, _parent2, _scopeId));
                ...
                if (loadingReponse.value) {
                  _push2(`<div class="flex items-center justify-center gap-3 py-8 text-slate-400" data-v-e97cebaa${_scopeId}><div class="w-5 h-5 border-2 border-slate-200 border-t-emerald-500 rounded-full animate-spin" data-v-e97cebaa${_scopeId}></div><span class="text-xs font-medium" data-v-e97cebaa${_scopeId}>Chargement du courrier de r\xE9ponse...</span></div>`);
                } else if (reponseData.value) {
                  _push2(`<div class="p-4 space-y-3" data-v-e97cebaa${_scopeId}><div class="grid grid-cols-1 gap-2" data-v-e97cebaa${_scopeId}><div class="flex items-start gap-3 p-3 bg-emerald-50 rounded-lg border border-emerald-100" data-v-e97cebaa${_scopeId}><div class="w-1 min-h-[2rem] rounded-full bg-emerald-400 shrink-0 self-stretch" data-v-e97cebaa${_scopeId}></div><div data-v-e97cebaa${_scopeId}><p class="text-[10px] font-bold text-emerald-500 uppercase tracking-wider mb-0.5" data-v-e97cebaa${_scopeId}>R\xE9f\xE9rence</p><p class="text-sm font-bold text-emerald-900" data-v-e97cebaa${_scopeId}>${ssrInterpolate(reponseData.value.reference || "Sans r\xE9f\xE9rence")}</p></div></div><div class="flex items-start gap-3 p-3 bg-slate-50 rounded-lg border border-slate-100" data-v-e97cebaa${_scopeId}><div class="w-1 min-h-[2rem] rounded-full bg-slate-300 shrink-0 self-stretch" data-v-e97cebaa${_scopeId}></div><div data-v-e97cebaa${_scopeId}><p class="text-[10px] font-bold text-slate-400 uppercase tracking-wider mb-0.5" data-v-e97cebaa${_scopeId}>Objet</p><p class="text-sm text-slate-800 leading-relaxed" data-v-e97cebaa${_scopeId}>${ssrInterpolate(reponseData.value.objet || "Non sp\xE9cifi\xE9")}</p></div></div></div><div class="grid grid-cols-2 md:grid-cols-3 gap-2" data-v-e97cebaa${_scopeId}><div class="p-2.5 bg-slate-50 rounded-lg border border-slate-100" data-v-e97cebaa${_scopeId}><p class="text-[10px] font-bold text-slate-400 uppercase tracking-wider mb-0.5" data-v-e97cebaa${_scopeId}>Destinataire</p><p class="text-xs font-semibold text-slate-800" data-v-e97cebaa${_scopeId}>${ssrInterpolate(reponseData.value.destinataire || "\u2014")}</p></div><div class="p-2.5 bg-slate-50 rounded-lg border border-slate-100" data-v-e97cebaa${_scopeId}><p class="text-[10px] font-bold text-slate-400 uppercase tracking-wider mb-0.5" data-v-e97cebaa${_scopeId}>Date de d\xE9part</p><p class="text-xs text-slate-800" data-v-e97cebaa${_scopeId}>${ssrInterpolate(formatDate(reponseData.value.date_depart) || "\u2014")}</p></div>`);
                  if (reponseData.value.service_emis) {
                    _push2(`<div class="p-2.5 bg-slate-50 rounded-lg border border-slate-100" data-v-e97cebaa${_scopeId}><p class="text-[10px] font-bold text-slate-400 uppercase tracking-wider mb-0.5" data-v-e97cebaa${_scopeId}>Service \xE9metteur</p><p class="text-xs text-slate-800" data-v-e97cebaa${_scopeId}>${ssrInterpolate(reponseData.value.service_emis)}</p></div>`);
                  } else {
                    _push2(`<!---->`);
                  }
                  if (reponseData.value.type_depart) {
                    _push2(`<div class="p-2.5 bg-slate-50 rounded-lg border border-slate-100" data-v-e97cebaa${_scopeId}><p class="text-[10px] font-bold text-slate-400 uppercase tracking-wider mb-0.5" data-v-e97cebaa${_scopeId}>Type de d\xE9part</p><p class="text-xs text-slate-800" data-v-e97cebaa${_scopeId}>${ssrInterpolate(reponseData.value.type_depart)}</p></div>`);
                  } else {
                    _push2(`<!---->`);
                  }
                  _push2(`</div><div class="pt-1" data-v-e97cebaa${_scopeId}>`);
                  if (reponseData.value.url) {
                    ...
                    if (!showReponseDoc.value) {
                      _push2(`<button class="inline-flex items-center gap-2 px-3.5 py-2 text-xs font-semibold text-emerald-700 bg-emerald-50 hover:bg-emerald-100 border border-emerald-200 rounded-lg transition-all" data-v-e97cebaa${_scopeId}>`);
                      _push2(ssrRenderComponent(_component_Icon, {
                        name: "i-heroicons-document-arrow-down",
                        class: "w-4 h-4"
                      }, null, _parent2, _scopeId));
                      ...
                      _push2(`<div class="mt-2 rounded-lg overflow-hidden border border-emerald-200" data-v-e97cebaa${_scopeId}>`);
                      _push2(ssrRenderComponent(__nuxt_component_2, {
                  ...
                  _push2(`<div class="flex items-center gap-2 m-4 p-3 bg-red-50 border border-red-100 rounded-lg text-xs text-red-600" data-v-e97cebaa${_scopeId}>`);
                  _push2(ssrRenderComponent(_component_Icon, {
              ...
              _push2(`</div><div class="px-6 py-4 border-t border-slate-100 shrink-0 flex justify-end" data-v-e97cebaa${_scopeId}>`);
      ...
      _push(`<div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-6" data-v-e97cebaa><div data-v-e97cebaa><h1 class="text-2xl font-bold text-slate-900 tracking-tight" data-v-e97cebaa>Courriers arriv\xE9s</h1><p class="text-sm text-slate-500" data-v-e97cebaa>Gestion et suivi des courriers entrants</p></div>`);
      ...
      _push(`</div>`);
      if (unref(loading)) {
        _push(`<div class="flex flex-col items-center justify-center py-20 gap-4 text-slate-500" data-v-e97cebaa><div class="w-8 h-8 border-4 border-slate-200 border-t-indigo-600 rounded-full animate-spin" data-v-e97cebaa></div><span class="text-sm font-medium" data-v-e97cebaa>Chargement des donn\xE9es...</span></div>`);
      } else if (unref(error)) {
        _push(`<div class="flex items-center gap-4 p-5 bg-red-50 border border-red-200 rounded-xl max-w-2xl mx-auto sm:mx-0" data-v-e97cebaa><svg class="w-8 h-8 text-red-600 shrink-0" viewBox="0 0 20 20" fill="currentColor" data-v-e97cebaa><path fill-rule="evenodd" d="M18 10a8 8 0 11-16 0 8 8 0 0116 0zm-7 4a1 1 0 11-2 0 1 1 0 012 0zm-1-9a1 1 0 00-1 1v4a1 1 0 102 0V6a1 1 0 00-1-1z" clip-rule="evenodd" data-v-e97cebaa></path></svg><div class="flex-1" data-v-e97cebaa><p class="text-sm font-bold text-red-900" data-v-e97cebaa>Erreur de chargement</p><p class="text-xs text-red-700" data-v-e97cebaa>${ssrInterpolate(unref(error))}</p></div><button class="px-4 py-2 text-xs font-bold text-red-600 bg-white border border-red-200 rounded-lg hover:bg-red-50 transition-colors shrink-0" data-v-e97cebaa> R\xE9essayer </button></div>`);
      } else {
        _push(ssrRenderComponent(_sfc_main$1, {
          ...
          "advanced-filters": withCtx(({ filters, onFilter }, _push2, _parent2, _scopeId) => {
            if (_push2) {
              _push2(`<div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4" data-v-e97cebaa${_scopeId}><!--[-->`);
              ssrRenderList([
                ...
                _push2(`<div data-v-e97cebaa${_scopeId}><label class="block text-[11px] font-bold text-slate-500 uppercase tracking-wider mb-1.5" data-v-e97cebaa${_scopeId}>${ssrInterpolate(field.label)}</label><input${ssrRenderAttr("value", filters[field.id])} placeholder="Filtrer..." class="w-full px-3 py-2 text-xs text-slate-900 bg-white border border-slate-300 rounded-lg focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500 outline-none transition-all" data-v-e97cebaa${_scopeId}></div>`);
              });
              _push2(`<!--]--></div>`);
            } else {
              return [
          ...
          actions: withCtx(({ item }, _push2, _parent2, _scopeId) => {
            var _a, _b, _c, _d, _e, _f;
            if (_push2) {
              _push2(`<div class="flex gap-1.5 justify-end" data-v-e97cebaa${_scopeId}><button title="Voir les d\xE9tails" class="inline-flex items-center justify-center w-8 h-8 bg-amber-50 text-amber-700 border-amber-100 rounded-md hover:bg-amber-200 hover:text-amber-900 transition-all group" data-v-e97cebaa${_scopeId}>`);
              _push2(ssrRenderComponent(_component_Icon, {
                name: "i-heroicons-eye",
                class: "w-4 h-4 group-hover:text-yellow-600"
              }, null, _parent2, _scopeId));
              _push2(`</button><button title="Affecter ce courrier" class="inline-flex items-center justify-center w-8 h-8 bg-sky-50 text-sky-700 border-sky-100 rounded-md hover:bg-sky-200 hover:text-sky-900 transition-all group" data-v-e97cebaa${_scopeId}>`);
              _push2(ssrRenderComponent(_component_Icon, {
                name: "i-heroicons-paper-airplane",
                class: "w-4 h-4 group-hover:text-blue-600"
              }, null, _parent2, _scopeId));
              _push2(`</button>`);
              if (!((_c = (_b = (_a = item._complete) == null ? void 0 : _a.document) == null ? void 0 : _b.reponses) == null ? void 0 : _c.length)) {
                _push2(`<button title="R\xE9pondre au courrier" class="inline-flex items-center justify-center w-8 h-8 bg-emerald-50 text-emerald-700 border-emerald-100 rounded-md hover:bg-emerald-200 hover:text-emerald-900 transition-all group" data-v-e97cebaa${_scopeId}>`);
                _push2(ssrRenderComponent(_component_Icon, {
                  name: "i-heroicons-arrow-uturn-right",
                ...
                _push2(`<div title="Ce courrier a d\xE9j\xE0 une r\xE9ponse" class="inline-flex items-center justify-center w-8 h-8 bg-green-50 text-green-500 border border-green-100 rounded-md cursor-default" data-v-e97cebaa${_scopeId}>`);
                _push2(ssrRenderComponent(_component_Icon, {
                  name: "i-heroicons-check-circle",
                  class: "w-4 h-4"
                }, null, _parent2, _scopeId));
              ...
              return [
                createVNode("div", { class: "flex gap-1.5 justify-end" }, [
                  ...
                  !((_f = (_e = (_d = item._complete) == null ? void 0 : _d.document) == null ? void 0 : _e.reponses) == null ? void 0 : _f.length) ? (openBlock(), createBlock("button", {
                    key: 0,
          ...
          "cell-source": withCtx(({ value }, _push2, _parent2, _scopeId) => {
            if (_push2) {
              _push2(`<span class="inline-flex px-2.5 py-1 text-[11px] font-bold rounded-full bg-indigo-50 text-indigo-700 border border-indigo-100" data-v-e97cebaa${_scopeId}>${ssrInterpolate(value)}</span>`);
            } else {
              return [
          ...
          "cell-reference": withCtx(({ value, item }, _push2, _parent2, _scopeId) => {
            if (_push2) {
              if (item.url) {
                _push2(`<button class="inline-flex items-center gap-1.5 px-2.5 py-0.5 text-xs font-medium text-blue-700 bg-blue-50 hover:bg-blue-100 border border-blue-200 rounded-md transition-all group"${ssrRenderAttr("title", `Ouvrir le document ${value}`)} data-v-e97cebaa${_scopeId}>`);
                _push2(ssrRenderComponent(_component_Icon, {
                  name: "i-heroicons-document-text",
                  class: "w-3.5 h-3.5 group-hover:scale-110 transition-transform"
                }, null, _parent2, _scopeId));
                _push2(`<span data-v-e97cebaa${_scopeId}>${ssrInterpolate(value)}</span>`);
                _push2(ssrRenderComponent(_component_Icon, {
                ...
                _push2(`<span class="inline-flex items-center px-2.5 py-0.5 text-xs font-medium text-gray-600 bg-gray-100 rounded-md" data-v-e97cebaa${_scopeId}>`);
                _push2(ssrRenderComponent(_component_Icon, {
                  name: "i-heroicons-document-text",
          ...
          "cell-priority": withCtx(({ value }, _push2, _parent2, _scopeId) => {
            if (_push2) {
              ...
              }, "inline-flex px-2.5 py-1 text-[11px] font-bold rounded-full border uppercase"])}" data-v-e97cebaa${_scopeId}>${ssrInterpolate(value || "Standard")}</span>`);
            } else {
              return [
                createVNode("span", {
...
const __nuxt_component_5 = /* @__PURE__ */ _export_sfc(_sfc_main, [["__scopeId", "data-v-e97cebaa"]]);

export { __nuxt_component_5 as default };
//# sourceMappingURL=CourriersArrivesListe-rply_gxW.mjs.map

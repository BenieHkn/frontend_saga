<template>
  <div class="smq-content">
    ...
    <SmqPageHeader overline="SMQ · FAC" title="Fiches d'Actions Correctives">
      <div class="qp-seg">
        <button v-for="f in FAC_STATUTS" :key="f.value" :class="{ active: filtreStatut === f.value }" @click="filtreStatut = f.value; charger()">{{ f.label }}</button>
      </div>
    </SmqPageHeader>
    ...
    <div v-else class="qp-tablecard">
      <table class="qp-table">
        <thead>
          <tr>
            <th style="width:120px">Fiche N°</th>
            <th>Indicateur</th>
        ...
        <tbody>
          ...
          <tr v-if="!facList.length">
            <td colspan="7" class="text-center py-10 text-sm" style="color:var(--qp-fg-3)">Aucune fiche d'action corrective.</td>
          </tr>
        </tbody>
      </table>
    </div>
        ...
        <div class="px-5 py-4 flex flex-col gap-5">
          ...
          <div class="grid gap-4" style="grid-template-columns:1fr 1fr">
            <div class="qp-field">
              ...
              <input v-model="form.date_etablissement" type="date" class="qp-input qp-input--mono" :disabled="facCloturee" />
            </div>
            <div class="qp-field">
              <label class="qp-label">Par</label>
              <input v-model="form.etabli_par" class="qp-input" placeholder="Nom" :disabled="facCloturee" />
            </div>
          </div>
          ...
          <div class="rounded-lg overflow-hidden" style="border:1px solid var(--qp-border-2)">
            ...
            <div class="p-4 flex flex-col gap-4">
              <div class="qp-field">
                ...
                <textarea v-model="form.description_nc" class="qp-textarea" rows="3" :disabled="facCloturee" />
              </div>
              <div class="qp-field">
                <label class="qp-label">Action effectuée pour maîtriser et corriger la non-conformité</label>
                <textarea v-model="form.action_maitrise" class="qp-textarea" rows="2" :disabled="facCloturee" />
              </div>
              <div class="qp-field">
                <label class="qp-label">Cause(s) du dysfonctionnement ou de la non-conformité</label>
                <textarea v-model="form.causes" class="qp-textarea" rows="2" :disabled="facCloturee" />
              </div>
              <div class="qp-field">
              ...
              <div class="grid gap-4" style="grid-template-columns:1fr 1fr">
                ...
                <div class="qp-field">
                  ...
                  <input v-model="form.date_previsionnelle" type="date" class="qp-input qp-input--mono" :disabled="facCloturee" />
                </div>
                <div class="qp-field">
                  <label class="qp-label">Critère d'efficacité</label>
                  <input v-model="form.critere_efficacite" class="qp-input" placeholder="Ex. : indicateur ≥ cible 2 périodes" :disabled="facCloturee" />
                </div>
                <div class="qp-field">
                  <label class="qp-label">Date prévue pour l'examen des effets</label>
                  <input v-model="form.date_examen_effets" type="date" class="qp-input qp-input--mono" :disabled="facCloturee" />
                </div>
              </div>
            </div>
          </div>
            ...
            <div v-if="peutRemplirRQ" class="p-4 flex flex-col gap-4">
              <div class="grid gap-4" style="grid-template-columns:1fr 1fr">
                <div class="qp-field">
                  ...
                  <input v-model="form.date_verification" type="date" class="qp-input qp-input--mono" :disabled="facCloturee" />
                </div>
                <div class="qp-field">
                  <label class="qp-label">Conclusion</label>
                  <select v-model="form.conclusion_efficacite" class="qp-select" :disabled="facCloturee">
              ...
              <div v-if="form.fiche_cloturee === false" class="grid gap-4" style="grid-template-columns:1fr 1fr">
                <div class="qp-field">
                  ...
                  <select v-model="form.ouvrir_nouvelle_fiche" class="qp-select" :disabled="facCloturee">
                    <option :value="null">— Choisir —</option>
                    <option :value="true">Oui</option>
                    <option :value="false">Non</option>
                  </select>
                ...
                <div v-if="form.ouvrir_nouvelle_fiche" class="qp-field">
                  ...
                  <input v-model="form.num_nouvelle_fiche" class="qp-input qp-input--mono" :disabled="facCloturee" />
                </div>
              </div>
              <div class="qp-field">
                <label class="qp-label">Commentaire(s) éventuel(s)</label>
                <textarea v-model="form.commentaires_rq" class="qp-textarea" rows="2" :disabled="facCloturee" />
              </div>
            </div>
...
const statutStyle = (s) => ({
  ouvert:   'background:var(--qp-n-100);color:var(--qp-fg-2);border:1px solid var(--qp-border-1)',
  en_cours: 'background:var(--qp-primary-50);color:var(--qp-primary-700);border:1px solid var(--qp-primary-200)',
  clos:     'background:var(--qp-success-50);color:var(--qp-success-700);border:1px solid var(--qp-success-200)',
}[s] ?? '')

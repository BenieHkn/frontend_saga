<template>
  <div class="min-h-screen bg-slate-100 p-6 font-sans">
    ...
    <div class="bg-white border border-slate-200 rounded-xl p-4 mb-5 shadow-sm flex flex-wrap gap-3 items-end">
      <div class="flex-1 min-w-[200px] max-w-sm">
        ...
        <div class="relative">
          ...
          <input v-model="filters.search" type="text" placeholder="Code ou libellé…"
      ...
      <div class="min-w-[200px]">
        ...
        <select v-else v-model="filters.type_document_id"
    ...
    <div v-else-if="viewMode === 'list'" class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
      <div v-for="modele in modeles" :key="modele.id"
        ...
        <div class="flex items-center justify-between px-3.5 pt-3.5 pb-2">
          <div class="flex items-center gap-1.5 flex-wrap">
            ...
            <span v-if="modele.est_defaut" class="inline-flex px-2 py-0.5 text-[10px] font-bold rounded-full bg-amber-50 text-amber-700 border border-amber-100 uppercase">Défaut</span>
          </div>
        ...
        <div class="px-3.5 cursor-pointer flex-1" @click="openPreview(modele)">
          <div class="bg-slate-50 border border-slate-100 rounded-lg p-3 mb-3 space-y-1.5">
            <div v-for="w in ['full','3/4','full','1/2','full','2/3']" :key="w" class="h-1.5 bg-slate-200 rounded-full"
              :style="`width:${w==='full'?'100%':w==='3/4'?'75%':w==='1/2'?'50%':'66%'}`" ></div>>
          </div>
      ...
      <table class="w-full border-collapse text-sm">
        <thead>
          <tr class="bg-slate-50">
            <th v-for="col in ['Libellé','Type','Actions']" :key="col"
        ...
        <tbody>
          <tr v-for="modele in modeles" :key="modele.id"
            ...
            <td class="px-4 py-3"><span class="inline-flex px-2.5 py-1 text-[11px] font-bold rounded-full bg-indigo-50 text-indigo-700 border border-indigo-100">{{ modele.type_document?.libelle || 'N/A' }}</span></td>
           
            <td class="px-4 py-3">
              <div class="flex items-center gap-1.5">
                <button class="inline-flex items-center justify-center w-7 h-7 rounded-lg bg-slate-50 text-slate-500 border border-slate-200 hover:bg-slate-100 transition-colors" @click="openPreview(modele)">
                  <svg width="12" height="12" viewBox="0 0 16 16" fill="none"><path d="M1 8s3-5 7-5 7 5 7 5-3 5-7 5-7-5-7-5z" stroke="currentColor" stroke-width="1.5"/><circle cx="8" cy="8" r="2" stroke="currentColor" stroke-width="1.5"/></svg>
                </button>
    ...
    <div v-if="showFormModal" class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4" @click.self="closeFormModal">
      <div class="bg-white rounded-xl shadow-xl w-full max-w-2xl max-h-[90vh] overflow-y-auto flex flex-col">
        <div class="flex items-center justify-between px-6 py-4 border-b border-gray-200 sticky top-0 bg-white z-10">
          <div>
            ...
            <p class="text-sm text-slate-500 mt-0.5">{{ editingModele ? `Édition de "${editingModele.libelle}"` : 'Créer un nouveau modèle de document' }}</p>
          </div>
        ...
        <div class="px-6 py-5 space-y-5 flex-1">
          <div>
            ...
            <div v-if="loadingTypes" class="flex items-center gap-2 py-2 px-3 border border-slate-300 rounded-lg bg-slate-50 text-xs text-slate-400">
              <div class="w-3.5 h-3.5 border-2 border-slate-200 border-t-indigo-500 rounded-full animate-spin flex-shrink-0" />Chargement des types…
            </div>
          ...
          <div class="grid grid-cols-2 gap-4">
            <div>
              ...
              <select v-model="form.orientation" class="w-full px-3 py-2 text-sm text-slate-900 bg-white border border-slate-300 rounded-lg outline-none cursor-pointer">
                <option value="portrait">Portrait</option><option value="paysage">Paysage</option>
              </select>
            </div>
            <div>
              <label class="block text-sm font-medium text-gray-700 mb-2">Format de page</label>
              <select v-model="form.format_page" class="w-full px-3 py-2 text-sm text-slate-900 bg-white border border-slate-300 rounded-lg outline-none cursor-pointer">
          ...
          <div v-if="editingModele" class="flex items-start gap-3 p-3 bg-amber-50 border border-amber-200 rounded-lg text-xs text-amber-800">
            ...
            <div><strong>Modèle chargé depuis la base de données.</strong> Utilisez <em>"Réinitialiser le template"</em> pour écraser avec la version à jour.</div>
          </div>
          <div>
            <div class="flex items-center justify-between mb-2">
              ...
              <div class="flex items-center gap-2">
                ...
                <button v-if="form.type_document_id && getTemplatePourType(form.type_document_id).template_html"
                  type="button" class="inline-flex items-center gap-1.5 px-2.5 py-1 text-[11px] font-semibold text-amber-700 bg-amber-50 border border-amber-200 rounded-md hover:bg-amber-100 transition-colors" @click="rechargerTemplate">
          ...
          <div class="flex items-center gap-6">
            <label class="flex items-center gap-2.5 cursor-pointer select-none">
              <button type="button" class="relative inline-flex items-center rounded-full transition-colors duration-200 focus:outline-none flex-shrink-0"
                :class="form.est_actif ? 'bg-indigo-600' : 'bg-slate-300'" style="width:38px;height:21px" @click="form.est_actif = !form.est_actif">
            ...
            <label class="flex items-center gap-2.5 cursor-pointer select-none">
              <button type="button" class="relative inline-flex items-center rounded-full transition-colors duration-200 focus:outline-none flex-shrink-0"
                :class="form.est_defaut ? 'bg-indigo-600' : 'bg-slate-300'" style="width:38px;height:21px" @click="form.est_defaut = !form.est_defaut">
        ...
        <div class="flex justify-end gap-3 px-6 py-4 border-t border-gray-200 bg-gray-50 rounded-b-xl sticky bottom-0">
          ...
          <UButton type="button" :loading="submitting" :disabled="submitting" @click="submitForm">
            <svg v-if="!submitting" class="w-4 h-4 mr-1.5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7" /></svg>
    ...
    <div v-if="showPreviewModal" class="fixed inset-0 bg-black bg-opacity-60 flex items-center justify-center z-50 p-4" @click.self="closePreview">
      <div class="bg-white rounded-xl shadow-xl w-full max-w-5xl max-h-[95vh] overflow-y-auto flex flex-col">
        ...
        <div class="px-6 pt-5 flex-1">
          <div class="bg-slate-100 border border-slate-200 rounded-xl overflow-hidden">
            <div class="flex items-center gap-5 px-4 py-2 bg-slate-50 border-b border-slate-200 text-[11px] font-semibold text-slate-500 flex-wrap">
              ...
              <span class="flex items-center gap-1.5 ml-auto"><span class="inline-block w-2.5 h-2.5 rounded-sm bg-indigo-100 border border-indigo-400"></span> Champ à renseigner</span>
            </div>
            <iframe ref="previewIframe" :srcdoc="previewFullHtml" class="w-full bg-white"
              style="min-height: 760px; border: none;" sandbox="allow-same-origin allow-scripts" />
          </div>
        </div>
        <div class="flex items-center justify-between px-6 py-4 border-t border-gray-200 bg-gray-50 rounded-b-xl sticky bottom-0 mt-5">
          <p class="text-xs text-slate-400 italic hidden sm:block">Renseignez les champs dans l'aperçu, puis générez le PDF.</p>
          <div class="flex gap-3 ml-auto">
            <button class="inline-flex items-center gap-2 px-5 py-2.5 text-sm font-semibold text-slate-700 bg-white border border-slate-300 rounded-lg hover:bg-slate-50 transition-colors" @click="closePreview">
              <svg class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"/></svg>
              Annuler
            </button>
            <button class="inline-flex items-center gap-2 px-5 py-2.5 text-sm font-semibold text-white bg-indigo-600 rounded-lg hover:bg-indigo-700 transition-colors shadow-sm disabled:opacity-60 disabled:cursor-not-allowed"
              :disabled="generatingPdf" @click="genererPdf">
              <svg v-if="!generatingPdf" class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z"/></svg>
              ...
              {{ generatingPdf ? 'Génération…' : 'Soumettre' }}
            </button>
          </div>
        </div>
      </div>
    ...
    <!-- MODAL PDF GÉNÉRÉ -->
    <div v-if="showPdfModal" class="fixed inset-0 bg-black bg-opacity-70 flex items-center justify-center z-[60] p-4" @click.self="closePdfModal">
      <div class="bg-white rounded-xl shadow-2xl w-full max-w-4xl max-h-[95vh] flex flex-col overflow-hidden">
        <div class="flex items-center justify-between px-6 py-4 border-b border-gray-200 bg-white sticky top-0 z-10">
          <div class="flex items-center gap-3">
            <div class="w-10 h-10 rounded-lg bg-red-50 border border-red-100 flex items-center justify-center flex-shrink-0">
            ...
            <div>
              <h3 class="text-lg font-semibold text-gray-900">Document généré</h3>
              <p class="text-sm text-slate-500">Vérifiez le document avant de l'envoyer</p>
            </div>
          </div>
          <button class="text-gray-400 hover:text-gray-600 transition-colors" @click="closePdfModal">
            <svg class="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12" /></svg>
          </button>
        </div>
        <div class="flex-1 overflow-auto bg-slate-100 p-4">
          <iframe v-if="pdfBlobUrl" :src="pdfBlobUrl"
            class="w-full rounded-lg border border-slate-200 shadow-sm bg-white" style="min-height: 680px; border: none;" />
          <div v-else class="flex flex-col items-center justify-center py-16 gap-4 text-slate-500">
        ...
        <div class="flex items-center justify-between px-6 py-4 border-t border-gray-200 bg-gray-50 sticky bottom-0">
          <p class="text-xs text-slate-400 italic hidden sm:block">Le PDF est prêt. Téléchargez-le ou envoyez-le directement.</p>
          <div class="flex gap-3 ml-auto">
            <button class="inline-flex items-center gap-2 px-4 py-2.5 text-sm font-semibold text-slate-700 bg-white border border-slate-300 rounded-lg hover:bg-slate-50 transition-colors" @click="closePdfModal">
              <svg class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M10 19l-7-7m0 0l7-7m-7 7h18"/></svg>
              Retour
            </button>
            <a v-if="pdfBlobUrl" :href="pdfBlobUrl" :download="`${previewModele?.code || 'document'}.pdf`"
              class="inline-flex items-center gap-2 px-4 py-2.5 text-sm font-semibold text-indigo-700 bg-indigo-50 border border-indigo-200 rounded-lg hover:bg-indigo-100 transition-colors">
              <svg class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 16v1a3 3 0 003 3h10a3 3 0 003-3v-1m-4-4l-4 4m0 0l-4-4m4 4V4"/></svg>
              Télécharger
            </a>
            <button class="inline-flex items-center gap-2 px-5 py-2.5 text-sm font-semibold text-white bg-emerald-600 rounded-lg hover:bg-emerald-700 transition-colors shadow-sm disabled:opacity-60 disabled:cursor-not-allowed"
              :disabled="transmitting" @click="transmettre">
              <svg v-if="!transmitting" class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 19l9 2-9-18-9 18 9-2zm0 0v-8"/></svg>
              <svg v-else class="w-4 h-4 animate-spin" fill="none" viewBox="0 0 24 24"><circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4"/><path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8v8H4z"/></svg>
              {{ transmitting ? 'Envoi…' : 'Envoyer' }}
            </button>
          </div>
        </div>
      </div>
    ...
    <UModal v-model="showDeleteModal">
      <div class="p-6">
        ...
        <div v-if="deletingModele" class="bg-slate-50 border border-slate-200 rounded-lg p-4 mb-4 space-y-2 text-sm">
          <div class="flex items-center justify-between"><span class="text-slate-600 font-medium">Code :</span><span class="font-semibold font-mono text-slate-900">{{ deletingModele.code }}</span></div>
          <div class="flex items-center justify-between"><span class="text-slate-600 font-medium">Libellé :</span><span class="font-semibold text-slate-900">{{ deletingModele.libelle }}</span></div>
          <div class="flex items-center justify-between"><span class="text-slate-600 font-medium">Type :</span><span class="font-semibold text-slate-900">{{ deletingModele.type_document?.libelle }}</span></div>
        </div>
...
// ── Formatage date ISO "YYYY-MM-DD" → "j mois aaaa" ────────────
...
// ════════════════════════════════════════════════════════════════
// TEMPLATES PRÉDÉFINIS — champs texte en <span contenteditable>
// ════════════════════════════════════════════════════════════════
const FIELD_STYLE = `border:none;border-bottom:1.5px dashed #6366f1;background:#eef2ff;border-radius:3px 3px 0 0;padding:0 3px;font-family:inherit;font-size:inherit;font-weight:inherit;color:#1a1a2e;outline:none;min-width:40px;display:inline;`

const TEMPLATES_PREDEFINIS = computed(() => ({
  'attestation': {
    entete_html: `<table width="100%" cellpadding="0" cellspacing="0" style="font-family:Arial,sans-serif;font-size:10px;border-bottom:1.5px solid #bbb;padding:8px 16px 6px;background:#fff;">
  <tr>
    <td width="40%" valign="top">
      <div style="display:flex;align-items:flex-start;gap:8px;">
        <img src="${appOrigin}/images/MEF.png" alt="Armoiries du Bénin" style="height:62px;flex-shrink:0;" />
      </div>
      <div style="border-bottom:1px solid #111;text-align:center;width:55%;margin:6px 0 0 50px;font-size:8.5px;font-weight:400;color:#111;line-height:1.6;padding-bottom:2px;">
        <div><span>-&nbsp;-</span></div>DIRECTION GÉNÉRALE DU MATÉRIEL<br>ET DE LA LOGISTIQUE
      </div>
      <div style="width:55%;margin-left:50px;margin-top:4px;font-size:11px;font-weight:bold;text-align:center;font-family:Arial,sans-serif;">Le Directeur Général</div>
      <div style="height:26px;"></div>
      <div style="font-size:11px;font-family:Arial,sans-serif;white-space:nowrap;">
        N°&nbsp;<input type="text" data-field="numero" placeholder="N° référence" style="width:60px;border:none;border-bottom:1px solid #000;background:transparent;font-family:inherit;font-size:inherit;outline:none;" />/MEF/SGM/DGML/DGR/SAP/SA
      </div>
    </td>
    <td width="20%" valign="top" align="center" style="padding-top:4px;">
      <img src="${appOrigin}/images/logoAB.png" alt="AB Certification ISO 9001:2015" style="height:62px;" />
    </td>
    <td width="40%" valign="top" align="right">
      <img src="${appOrigin}/images/logo_dgml.png" alt="Logo DGML" style="height:62px;" />
      <div style="margin-top:12px;font-size:11px;font-family:Arial,sans-serif;text-align:right;">
        Cotonou, le&nbsp;<input type="date" data-field="date_document" style="border:none;border-bottom:1.5px dashed #6366f1;background:#eef2ff;border-radius:3px 3px 0 0;padding:0 3px;font-family:inherit;font-size:inherit;color:#1a1a2e;outline:none;min-width:110px;" />
      </div>
    </td>
  </tr>
</table>`,

    template_html: `<div style="font-family:'Bookman Old Style',Bookman,'URW Bookman L',serif;font-size:13.5px;line-height:1.85;text-align:justify;padding:0 10px;">

  <div style="text-align:center;margin:22px 0 26px;">
    <div style="display:inline-block;position:relative;border:2.5px solid #111;border-radius:5px;padding:12px 52px;background:#fff;">
      <svg style="position:absolute;left:-22px;top:50%;transform:translateY(-50%);" width="24" height="48" viewBox="0 0 24 48" fill="none"><path d="M20 4 C4 4 4 24 20 24 C4 24 4 44 20 44" stroke="#111" stroke-width="2.2" fill="none"/></svg>
      <svg style="position:absolute;right:-22px;top:50%;transform:translateY(-50%) scaleX(-1);" width="24" height="48" viewBox="0 0 24 48" fill="none"><path d="M20 4 C4 4 4 24 20 24 C4 24 4 44 20 44" stroke="#111" stroke-width="2.2" fill="none"/></svg>
      <div style="font-size:16px;font-weight:900;letter-spacing:1.2px;text-transform:uppercase;color:#111;line-height:1.3;">ATTESTATION DE BONNE FIN<br>D'EXÉCUTION</div>
    </div>
  </div>

  <p style="text-indent:40px;margin-bottom:16px;">
    Je soussigné, <strong><span contenteditable="true" data-field="signataire_nom" style="${FIELD_STYLE}font-weight:bold;">Raoufou MAMAM</span></strong>,
    <strong><span contenteditable="true" data-field="signataire_titre" style="${FIELD_STYLE}font-weight:bold;">Directeur Général du Matériel et de la Logistique du Ministère de l'Economie et des Finances</span></strong>,
    atteste par la présente que
    <strong><span contenteditable="true" data-field="beneficiaire_civilite" style="${FIELD_STYLE}font-weight:bold;">Monsieur</span>&#160;<span contenteditable="true" data-field="beneficiaire_nom" style="${FIELD_STYLE}font-weight:bold;" data-placeholder="Nom complet"></span></strong>,
    <span contenteditable="true" data-field="beneficiaire_qualite" style="${FIELD_STYLE}" data-placeholder="Qualité / statut"></span>,
    directeur du cabinet <strong><span contenteditable="true" data-field="cabinet" style="${FIELD_STYLE}font-weight:bold;" data-placeholder="Nom du cabinet"></span></strong>,
    a exécuté en qualité de <span contenteditable="true" data-field="role_execute" style="${FIELD_STYLE}" data-placeholder="Rôle exécuté"></span>,
    une session de&#160;: «&#160;<strong><span contenteditable="true" data-field="intitule_formation" style="${FIELD_STYLE}font-weight:bold;" data-placeholder="Intitulé de la formation"></span></strong>&#160;»
    au profit des <span contenteditable="true" data-field="beneficiaires_groupe" style="${FIELD_STYLE}" data-placeholder="Groupe bénéficiaire"></span>
    sur la période du&#160;<strong><input type="date" data-field="date_debut" style="border:none;border-bottom:1.5px dashed #6366f1;background:#eef2ff;border-radius:3px 3px 0 0;padding:0 3px;font-family:inherit;font-size:inherit;font-weight:bold;color:#1a1a2e;outline:none;min-width:110px;" /></strong>
    au&#160;<strong><input type="date" data-field="date_fin" style="border:none;border-bottom:1.5px dashed #6366f1;background:#eef2ff;border-radius:3px 3px 0 0;padding:0 3px;font-family:inherit;font-size:inherit;font-weight:bold;color:#1a1a2e;outline:none;min-width:110px;" /></strong>.
  </p>

  <p style="text-indent:40px;margin-bottom:16px;">Il a réalisé avec professionnalisme et dévotion ces prestations conformément à nos attentes.</p>

  <p style="text-indent:40px;margin-bottom:24px;">En foi de quoi, la présente attestation lui est délivrée pour servir et valoir ce que de droit.</p>

  <div style="text-align:right;padding-right:40px;margin-top:10px;">
    <div style="position:relative;display:inline-block;text-align:center;min-width:180px;">
      <img src="{{cachet}}" alt="Cachet" style="position:absolute;left:-50px;top:-10px;height:90px;opacity:0.80;z-index:1;" onerror="this.style.display='none'" />
      <img src="{{signature}}" alt="Signature" style="position:relative;height:55px;z-index:2;" onerror="this.style.display='none'" />
      <div style="font-weight:bold;font-size:13.5px;margin-top:6px;">
        <span contenteditable="true" data-field="signataire_nom_bas" style="${FIELD_STYLE}font-weight:bold;">Raoufou MAMAM</span>
      </div>
    </div>
  </div>
    ...
    pied_page_html: `<div style="font-family:Arial,sans-serif;border-top:1.5px solid #aaa;padding-top:5px;text-align:center;font-size:9px;color:#333;line-height:1.5;background:#fafafa;">
  <div style="margin-bottom:2px;color:#ccc;letter-spacing:3px;font-size:7.5px;">— — —</div>
  368, Avenue Pape Jean Paul II, 01 BP 302 Cotonou &nbsp;&#9742;&nbsp;21 30 61 69 / 21 30 85 20<br>
  &#9993;&nbsp;secretariat.dgml@finances.bj &nbsp;–&nbsp;secretariat@dgml.bj &nbsp;–&nbsp;<strong>www.dgml.bj</strong><br>
...
const defaultForm = () => ({
  type_document_id: '', code: '', libelle: '', template_html: '', entete_html: '', pied_page_html: '',
  orientation: 'portrait', format_page: 'A4', marge_haut: 20, marge_bas: 10, marge_gauche: 25, marge_droite: 25,
  signataire: '', fonction_signataire: '', est_actif: true, est_defaut: false,
})
const form = reactive(defaultForm())

watch(() => form.type_document_id, (newTypeId) => {
  if (editingModele.value || !newTypeId) return
...
function openCreateModal() {
  editingModele.value = null; Object.assign(form, defaultForm())
  errors.value = {}; formError.value = null; showFormModal.value = true
}

function openEdit(modele) {
  editingModele.value = modele
  Object.assign(form, {
    type_document_id: modele.type_document_id, code: modele.code, libelle: modele.libelle,
    template_html: modele.template_html, entete_html: modele.entete_html ?? '',
    pied_page_html: modele.pied_page_html ?? '', orientation: modele.orientation,
    format_page: modele.format_page, marge_haut: modele.marge_haut, marge_bas: modele.marge_bas,
  ...
  errors.value = {}; formError.value = null; activeMenu.value = null; showFormModal.value = true
}
...
async function submitForm() {
  ...
  const url    = editingModele.value ? `${config.public.apiBase}/modeles-document/${editingModele.value.id}` : `${config.public.apiBase}/modeles-document`
  ...
  try {
    ...
    if (err.status === 422 || err.statusCode === 422) { errors.value = err.data?.errors ?? {}; formError.value = err.data?.message || 'Erreurs de validation.' }
    else { formError.value = err.data?.message || err.message || 'Erreur serveur'; useToast().add({ title: 'Erreur', description: formError.value, color: 'red' }) }
...
// ════════════════════════════════════════════════════════════════
// BUILD PREVIEW HTML — style inline pour les <span contenteditable>
// ════════════════════════════════════════════════════════════════
function buildFullPreviewHtml(modele) {
  if (!modele) return ''
  const mH = modele.marge_haut ?? 20, mB = modele.marge_bas ?? 10
  const mG = modele.marge_gauche ?? 25, mD = modele.marge_droite ?? 25
  return `<!DOCTYPE html><html lang="fr"><head><meta charset="UTF-8">
<style>
  *,*::before,*::after{box-sizing:border-box;margin:0;padding:0;}
  body{font-family:'Bookman Old Style',Bookman,'URW Bookman L',serif;font-size:13.5px;color:#1a1a2e;background:#fff;width:210mm;min-height:297mm;display:flex;flex-direction:column;}
  ...
  [data-field]{display:inline;border:none;border-bottom:1.5px dashed #6366f1;background:#eef2ff;border-radius:3px 3px 0 0;padding:0 3px;font-family:inherit;font-size:inherit;font-weight:inherit;color:#1a1a2e;outline:none;cursor:text;}
  [data-field]:hover{background:#e0e7ff;}
  [data-field]:focus{background:#fff;border-bottom-color:#4f46e5;}
  [contenteditable][data-field]:empty::before{content:attr(data-placeholder);color:#94a3b8;font-style:italic;font-weight:normal;}
  input[data-field]{display:inline-block;width:auto;border:none;border-bottom:1.5px dashed #6366f1;background:#eef2ff;border-radius:3px 3px 0 0;padding:0 3px;font-family:inherit;font-size:inherit;color:#1a1a2e;outline:none;vertical-align:baseline;}
  input[data-field]:focus{background:#fff;border-bottom-color:#4f46e5;}
</style></head><body>
  <div class="doc-header">${modele.entete_html ?? ''}</div>
  <div class="doc-body">${modele.template_html ?? ''}</div>
  <div class="doc-footer">${modele.pied_page_html ?? ''}</div>
</body></html>`
}

function openPreview(modele) {
  previewModele.value = modele; previewFullHtml.value = buildFullPreviewHtml(modele)
  activeMenu.value = null; showPreviewModal.value = true
}

function closePreview() { showPreviewModal.value = false; previewModele.value = null; previewFullHtml.value = '' }

// ════════════════════════════════════════════════════════════════
// COLLECTER DONNÉES — lit <span contenteditable> ET <input>
// ════════════════════════════════════════════════════════════════
function collecterDonnees() {
  ...
  if (iframeDoc) {
    iframeDoc.querySelectorAll('[data-field]').forEach(el => {
      ...
      let valeur = ''
      if (el.tagName === 'INPUT') {
        valeur = el.value.trim()
        if (el.type === 'date' && valeur) valeur = formatDateFr(valeur)
      } else {
        // <span contenteditable>
        valeur = (el.innerText || el.textContent || '').trim()
      }
      donnees[key] = valeur
    })
  }
...
// ════════════════════════════════════════════════════════════════
// GÉNÉRATION PDF — fetch() natif pour blob
// ════════════════════════════════════════════════════════════════
async function genererPdf() {
  generatingPdf.value = true
  try {
    const donnees = collecterDonnees()
    pdfDonnees.value = donnees

    const response = await fetch(
      `${config.public.apiBase}/modeles-document/${previewModele.value.id}/generer-pdf`,
      {
        method:  'POST',
        headers: {
    ...
    if (!ct.includes('application/pdf')) throw new Error(`Réponse inattendue : ${ct}`)
    ...
    useToast().add({ title: 'Erreur de génération PDF', description: err.message || 'Erreur serveur', color: 'red' })
...
function closePdfModal() {
  ...
  pdfDonnees.value = {}
  if (previewModele.value) showPreviewModal.value = true
}
...
async function transmettre() {
  ...
  try {
    await $fetch(
      ...
      { method: 'POST', headers: { ...getHeaders(), 'Content-Type': 'application/json' }, body: JSON.stringify({ donnees: pdfDonnees.value }) }
    )
    useToast().add({ title: 'Transmis avec succès', description: 'Le document a été envoyé.', color: 'green' })
    if (pdfBlobUrl.value) URL.revokeObjectURL(pdfBlobUrl.value)
    pdfBlobUrl.value = null; pdfDonnees.value = {}
    showPdfModal.value = false; previewModele.value = null; previewFullHtml.value = ''
  } catch (err) {
    useToast().add({ title: 'Erreur de transmission', description: err.data?.message || err.message || 'Erreur serveur', color: 'red' })
  } finally { transmitting.value = false }
}

function toggleMenu(id) { activeMenu.value = activeMenu.value === id ? null : id }
function closeMenuOnOutside(e) { if (!e.target.closest('[data-menu]')) activeMenu.value = null }

function rechargerTemplate() {
  const tpl = getTemplatePourType(form.type_document_id)
  if (!tpl.template_html) return
  form.entete_html = tpl.entete_html ?? ''; form.template_html = tpl.template_html; form.pied_page_html = tpl.pied_page_html ?? ''
  if (editingModele.value) useToast().add({ title: 'Template réinitialisé', description: 'Sauvegardez pour persister.', color: 'amber' })
}

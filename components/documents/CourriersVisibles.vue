        ...
        <div v-if="selectedCourrier" class="flex flex-col max-h-[90vh] bg-white rounded-xl overflow-hidden">
                ...
                <div class="relative flex items-center gap-3">
                    ...
                    <div>
                        ...
                        <div class="flex items-center gap-2 mt-0.5">
                            <span class="text-xs text-orange-200 font-medium">{{ selectedCourrier.numero_enreg || '—'
                                }}</span>
            ...
            <div class="overflow-y-auto flex-1 p-5 space-y-4 bg-slate-50/50">
                <section class="bg-white rounded-2xl border border-slate-200/80 overflow-hidden shadow-sm">
                    ...
                    <div
                        class="flex items-center justify-between px-4 py-3 bg-gradient-to-r from-orange-50 to-amber-50 border-b border-orange-100">
                        <div class="flex items-center gap-2">
                            ...
                            <span class="text-[11px] font-bold text-orange-700 uppercase tracking-widest">Courrier
                                départ</span>
                        </div>

                        <!-- Bouton charger le document -->
                        <div
                            v-if="selectedCourrier._raw?.document?.url && selectedCourrier._raw.document.url !== 'Inconnu'">
                            <button v-if="!docFileLoaded && !docFileLoading && !docFileError" @click="loadDocFile"
                                class="inline-flex items-center gap-1.5 px-3 py-1.5 text-xs font-semibold text-orange-700 bg-orange-50 hover:bg-orange-100 border border-orange-200 rounded-lg transition-all">
                            ...
                            <div v-else-if="docFileLoading"
                                ...
                                <div
                                    class="w-3.5 h-3.5 border-2 border-slate-200 border-t-orange-500 rounded-full animate-spin">
                                </div>
                                Chargement...
                            </div>
                    ...
                    <div class="p-4 space-y-3">
                        <div class="grid grid-cols-1 gap-2">
                            <div
                                class="flex items-stretch gap-0 rounded-xl overflow-hidden border border-orange-100 hover:border-orange-200 transition-colors">
                                ...
                                <div class="flex-1 p-3 bg-gradient-to-r from-orange-50 to-transparent">
                                    <p class="text-[10px] font-bold text-orange-400 uppercase tracking-wider mb-0.5">
                                        Référence</p>
                            ...
                            <div
                                ...
                                <div class="flex-1 p-3 bg-gradient-to-r from-amber-50 to-transparent">
                                    <p class="text-[10px] font-bold text-amber-500 uppercase tracking-wider mb-0.5">
                                        Objet</p>
                        ...
                        <div class="grid grid-cols-2 md:grid-cols-3 gap-2">
                            <div
                                ...
                                <p
                                    class="text-[9px] font-bold text-slate-400 uppercase tracking-wider mb-1 flex items-center gap-1">
                                    <span class="w-1.5 h-1.5 rounded-full bg-orange-300 inline-block"></span>Service
                                    émetteur
                                </p>
                                <p class="text-xs font-semibold text-slate-800">{{ selectedCourrier.service_emis || '—'
                                    }}</p>
                            </div>
                            <div
                                class="p-2.5 bg-white rounded-xl border border-slate-100 hover:border-slate-200 hover:shadow-sm transition-all">
                                ...
                                <p class="text-xs font-semibold text-slate-800">{{ selectedCourrier.destinataire || '—'
                                    }}</p>
                            </div>
                            <div
                                class="p-2.5 bg-white rounded-xl border border-slate-100 hover:border-slate-200 hover:shadow-sm transition-all">
                                <p
                                    class="text-[9px] font-bold text-slate-400 uppercase tracking-wider mb-1 flex items-center gap-1">
                                    <span class="w-1.5 h-1.5 rounded-full bg-slate-300 inline-block"></span>Type de
                                    départ
                                </p>
                            ...
                            <div
                                ...
                                <p
                                    class="text-[9px] font-bold text-slate-400 uppercase tracking-wider mb-1 flex items-center gap-1">
                                    <span class="w-1.5 h-1.5 rounded-full bg-slate-300 inline-block"></span>Date de
                                    départ
                                </p>
                                <p class="text-xs text-slate-800">{{ selectedCourrier.date_depart || '—' }}</p>
                            </div>
                            <div v-if="selectedCourrier.date_enreg"
                                class="p-2.5 bg-white rounded-xl border border-slate-100 hover:border-slate-200 hover:shadow-sm transition-all">
                                <p
                                    class="text-[9px] font-bold text-slate-400 uppercase tracking-wider mb-1 flex items-center gap-1">
                                    <span class="w-1.5 h-1.5 rounded-full bg-slate-300 inline-block"></span>Date
                                    d'enregistrement
                                </p>
                            ...
                            <div v-if="selectedCourrier.type_document"
                                ...
                                <p
                                    class="text-[9px] font-bold text-slate-400 uppercase tracking-wider mb-1 flex items-center gap-1">
                                    <span class="w-1.5 h-1.5 rounded-full bg-green-300 inline-block"></span>Type de
                                    document
                                </p>
                        ...
                        <div v-if="selectedCourrier.initiateurs"
                            ...
                            <div class="flex-1 p-3 bg-gradient-to-r from-blue-50 to-transparent">
                                <p class="text-[10px] font-bold text-blue-400 uppercase tracking-wider mb-1.5">
                                    Initiateurs</p>
            ...
            <div class="space-y-4">
                <div class="flex flex-wrap gap-3">
                    ...
                    <div class="flex-1 min-w-[200px]">
                        <label
                            class="block text-[11px] font-bold text-slate-500 uppercase tracking-wider mb-1.5">Objet</label>
                    ...
                    <div class="flex-1 min-w-[150px]">
                        <label
                            class="block text-[11px] font-bold text-slate-500 uppercase tracking-wider mb-1.5">Service
                            émetteur</label>
                ...
                <div class="flex flex-wrap gap-3">
                    <div class="flex-1 min-w-[150px]">
                        <label class="block text-[11px] font-bold text-slate-500 uppercase tracking-wider mb-1.5">Date
                            de départ (début)</label>
                    ...
                    <div class="flex-1 min-w-[140px]">
                        <label class="block text-[11px] font-bold text-slate-500 uppercase tracking-wider mb-1.5">Type
                            de départ</label>
                    ...
                    <div class="flex-1 min-w-[140px]">
                        <label
                            class="block text-[11px] font-bold text-slate-500 uppercase tracking-wider mb-1.5">Confidentialité</label>
        ...
        <template #cell-reference="{ value, item }">
            <div class="w-full">
                <button v-if="item._raw?.document?.url && item._raw.document.url !== 'Inconnu'"
                    @click="handleViewDetails(item)" :disabled="openingDocumentId === item.id"

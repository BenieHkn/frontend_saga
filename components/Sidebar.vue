    ...
    <nav class="flex-1 px-3 py-6 space-y-2 overflow-y-auto no-scrollbar">
      ...
      <div v-if="peutVoirConfig()" class="pt-4 mt-4 border-t border-gray-200">
        <button @click="toggleSettings"
          ...
          <Icon v-if="isExpanded" :name="settingsMenuOpen ? 'heroicons:chevron-up' : 'heroicons:chevron-down'"
            class="h-4 w-4 opacity-60" />
        </button>
        <div v-if="settingsMenuOpen && isExpanded" class="ml-10 space-y-2 mt-2 animate-slide-down">
          <NuxtLink to="/entites" class="block text-sm px-3 py-2 rounded-lg font-bold transition-colors"
            :class="route.path === '/entites' ? 'bg-gradient-to-br from-emerald-700 to-blue-800 text-white shadow-lg' : 'text-gray-700 hover:text-emerald-600 hover:bg-gray-100'">
            Entités
          </NuxtLink>
          <NuxtLink to="/utilisateurs" class="block text-sm px-3 py-2 rounded-lg font-bold transition-colors"
            :class="route.path === '/utilisateurs' ? 'bg-gradient-to-br from-emerald-700 to-blue-800 text-white shadow-lg' : 'text-gray-700 hover:text-emerald-600 hover:bg-gray-100'">
            Utilisateurs
          </NuxtLink>
          <NuxtLink to="/point-critique" class="block text-sm px-3 py-2 rounded-lg font-bold transition-colors"
            :class="route.path === '/point-critique' ? 'bg-gradient-to-br from-emerald-700 to-blue-800 text-white shadow-lg' : 'text-gray-700 hover:text-emerald-600 hover:bg-gray-100'">
            Point Critique
          </NuxtLink>
          <NuxtLink to="/interim" class="block text-sm px-3 py-2 rounded-lg font-bold transition-colors"
